...
def _format_help() -> str:
    return "\n".join(
        (
            ...
            "  18+ 模式只接受使用者已提供的完整劇本，使用本機 Qwen 與本機 renderer；不會替你擴寫，也不會呼叫成人產圖或外部媒體 provider。",
            "  角色、情緒與動作標籤只顯示在字幕，不會念出來。",
            "  字幕範例：",
            "  小美（走到小王面前）",
            "  「你好。」",
            ...
            "",
            "配音語氣（tone 可省略）",
            "- 一般語氣可直接描述為溫暖／開心／疑惑等自然概念；可以省略 tone，系統會依台詞的情緒、動作與節奏自動對應。",
            "- 節奏可描述為慢速／從容／自然／快速；系統只套用有上限的速度與停頓，不會改寫台詞。",
            "- 親密／挑逗／害羞等成人語氣，只能在有效的成人露骨內容設定中使用。",
            "- 角色、情緒與動作會顯示在字幕，但不會進入朗讀文字。",
            "- 語氣不會暗中加入呼吸、喘聲、笑聲或其他音效；只有原文真的寫出的字才會朗讀。",
            "- 多個省略號會轉成有上限的短停頓，不會形成異常長靜音。",
            "",
