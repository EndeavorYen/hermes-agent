...
class TestPluginDispatch:
    ...
    def test_handle_image_generate_rejects_prompt_builder_brief_without_provider_call(self, monkeypatch, tmp_path):
        from tools import image_generation_tool
        from agent import image_gen_registry as registry_module
        from hermes_cli import plugins as plugins_module
            ...
            image_generation_tool._handle_image_generate(
                {
                    "prompt": (
                        ...
                        "請翻 prompt 庫，幫我把這版改成更性感的服裝。"
                        "巨乳，水蛇腰，翹臀，蜜大腿，大長腿，絲足"
                    ),
                    "reference_image_urls": ["/tmp/ref-1.png", "/tmp/ref-2.png"],
                }
            )
        )
