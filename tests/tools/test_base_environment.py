...
class TestAtomicSnapshotWrite:
    ...
    def test_temp_path_uses_mktemp_not_shell_pid(self):
        """Use ``mktemp`` because ``$$`` is shared by background subshells and
        ``$BASHPID`` is absent from macOS' system Bash 3.2."""
        env = _TestableEnv()
        env._snapshot_ready = True
        wrapped = env._wrap_command("echo hi", "/tmp")
    ...
    def test_temp_path_template_is_quoted(self):
        """The mktemp template stays one shell word when paths contain spaces."""
        env = _TestableEnv()
        env._snapshot_ready = True
...
class TestAtomicSnapshotConcurrencyBehavioral:
    ...
    that would corrupt PATH.  Crucially it uses ``mktemp`` so every writer is
    unique even under macOS' Bash 3.2, where ``$BASHPID`` is unavailable.
    """
    ...
    def test_concurrent_writes_never_tear_the_snapshot(self, tmp_path):
        ...
        writer = (
            ...
            f"__hermes_snap_tmp=$(mktemp {_q(snap + '.tmp.XXXXXX')}) || continue; "
    ...
    def test_failed_export_does_not_destroy_good_snapshot(self, tmp_path):
        ...
        script = (
            f"__hermes_snap_tmp=$(mktemp {bad_tmp}) || true; "
            f"{{ export -p > \"$__hermes_snap_tmp\" && "
            f"mv -f \"$__hermes_snap_tmp\" {_q(snap)}; }} "
            f"2>/dev/null || rm -f \"$__hermes_snap_tmp\" 2>/dev/null || true"
        )
