...
@pytest.mark.asyncio
async def test_visual_package_blocks_story_video_video_body_before_provider_calls(monkeypatch):
    from tools import visual_package_tool

    def fail_generate_image(**kwargs):
        raise AssertionError("story-video video body must not use visual package image generation")

    def fail_generate_video(**kwargs):
        raise AssertionError("story-video video body must not use visual package video generation")
    ...
    payload = json.loads(
        await visual_package_tool._handle_visual_package_generate(
            {
                "prompt": "故事影片：恐龍起源科普影片，大概 5mins，真實照片風格。",
                "include_image": True,
                "include_video": True,
                "image_provider": "xai",
            }
