...
def test_image_generate_parallel_worker_cap_defaults_to_four():
    runnable_calls = [
        (0, _tool_call("image_generate", {"prompt": "one"}, "img_1"), "image_generate", {}),
        (1, _tool_call("image_generate", {"prompt": "two"}, "img_2"), "image_generate", {}),
        (2, _tool_call("image_generate", {"prompt": "three"}, "img_3"), "image_generate", {}),
        (3, _tool_call("image_generate", {"prompt": "four"}, "img_4"), "image_generate", {}),
        (4, _tool_call("image_generate", {"prompt": "five"}, "img_5"), "image_generate", {}),
    ]

    with patch("hermes_cli.config.load_config", return_value={}):
        assert run_agent._max_workers_for_tool_batch(runnable_calls) == 4
...
def test_image_generate_parallel_worker_cap_can_be_configured_lower():
    ...
    with patch(
        ...
        assert run_agent._max_workers_for_tool_batch(runnable_calls) == 1
