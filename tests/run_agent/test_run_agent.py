...
class TestRunConversation:
    ...
    def test_run_conversation_writes_reflection_artifact_for_tool_heavy_success(self, agent):
        self._setup_agent(agent)
        agent._reflection_enabled = True
        agent._reflection_min_tool_turns = 1
        tc = _mock_tool_call(name="web_search", arguments="{}", call_id="c1")
        resp1 = _mock_response(content="", finish_reason="tool_calls", tool_calls=[tc])
        resp2 = _mock_response(content="Done searching", finish_reason="stop")
        agent.client.chat.completions.create.side_effect = [resp1, resp2]
        ...
        with (
            ...
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
            patch("run_agent.append_reflection_record") as mock_append_reflection,
        ):
            result = agent.run_conversation("search something")

        assert result["completed"] is True
        mock_append_reflection.assert_called_once()
        reflection_record = mock_append_reflection.call_args.args[0]
        assert reflection_record["session_id"] == agent.session_id
        assert reflection_record["outcome"]["completed"] is True
        assert reflection_record["tools"]["unique"] == ["web_search"]

    def test_run_conversation_reflection_records_background_review_metadata(self, agent_with_memory_tool):
        ...
        tc = _mock_tool_call(name="web_search", arguments="{}", call_id="c1")
        resp1 = _mock_response(content="", finish_reason="tool_calls", tool_calls=[tc])
        resp2 = _mock_response(content="Final answer", finish_reason="stop")
        agent.client.chat.completions.create.side_effect = [resp1, resp2]
        ...
        with (
            ...
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
            patch.object(agent, "_spawn_background_review") as mock_spawn_background_review,
            patch("run_agent.append_reflection_record") as mock_append_reflection,
        ):
            result = agent.run_conversation("hello")

        assert result["completed"] is True
        mock_spawn_background_review.assert_called_once()
        reflection_record = mock_append_reflection.call_args.args[0]
        assert reflection_record["post_response_review"] == {"memory": True, "skills": False, "queued": True}
    ...
    def test_run_conversation_writes_reflection_artifact_for_interrupted_turn(self, agent):
        self._setup_agent(agent)
        agent._reflection_enabled = True
        ...
        mock_append_reflection.assert_called_once()
        reflection_record = mock_append_reflection.call_args.args[0]
        assert reflection_record["outcome"]["interrupted"] is True
        assert "reflection" in reflection_record["affordance"]["needs"]

    def test_run_conversation_interrupted_reflection_marks_background_review_not_queued(self, agent_with_memory_tool):
        agent = agent_with_memory_tool
        self._setup_agent(agent)
        agent._reflection_enabled = True
        agent._memory_store = MagicMock()
        ...
        assert result["interrupted"] is True
        mock_spawn_background_review.assert_not_called()
        reflection_record = mock_append_reflection.call_args.args[0]
        assert reflection_record["post_response_review"] == {"memory": True, "skills": False, "queued": False}

    def test_spawn_background_review_writes_review_event(self, agent):
        class FakeReviewAgent:
            def __init__(self, **kwargs):
                self._session_messages = [
                    {"role": "tool", "content": json.dumps({"success": True, "target": "memory", "message": "Entry added"})}
                ]
        ...
        with (
            ...
            patch("run_agent.append_background_review_event") as mock_append_background_review_event,
        ):
            agent._spawn_background_review(messages_snapshot=[{"role": "user", "content": "hello"}], review_memory=True)

        mock_append_background_review_event.assert_called_once()
        event = mock_append_background_review_event.call_args.args[0]
        assert event["session_id"] == agent.session_id
        assert event["review_memory"] is True
        assert event["review_skills"] is False
        assert event["actions"] == ["Memory updated"]
