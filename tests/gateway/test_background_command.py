...
class TestRunBackgroundTask:
    ...
    async def test_media_files_routed_by_type(self, monkeypatch):
        ...
        try:
            ...
            assert mock_adapter.send_voice.call_args.kwargs["audio_path"] == str(
                Path(_ogg).resolve()
            )
            mock_adapter.send_video.assert_called_once()
            assert mock_adapter.send_video.call_args.kwargs["video_path"] == str(
                Path(_mp4).resolve()
            )
            mock_adapter.send_image_file.assert_called_once()
            assert mock_adapter.send_image_file.call_args.kwargs["image_path"] == str(
                Path(_png).resolve()
            )
            mock_adapter.send_document.assert_called_once()
            assert mock_adapter.send_document.call_args.kwargs["file_path"] == str(
                Path(_pdf).resolve()
            )
        finally:
