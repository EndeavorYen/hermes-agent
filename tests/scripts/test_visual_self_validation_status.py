...
def _scheduled_report(*, success=True, live_decision="run"):
    ...
    return {
        ...
        "automation": {
            ...
            "artifact_path": "/private/mock-hermes-home/cache/images/private.jpg",
        },
        "self_review": {
            "privacy_safe": True,
...
def test_visual_self_validation_status_summarizes_latest_live_report(tmp_path):
    ...
    assert "/private/mock-hermes-home" not in encoded
...
def test_visual_self_validation_status_reports_unresolved_live_operator_setup(tmp_path):
    from scripts.visual_self_validation_status import build_visual_self_validation_status
    ...
    payload["live_policy"] = {
        ...
        "missing_env_vars": ["FAL_KEY"],
        "operator_setup_actions": operator_setup_actions,
        "action_types": ["configure_video_fallback_provider"],
    }
    payload["summary"]["feedback_action_types"] = ["configure_video_fallback_provider"]
    ...
    assert status["live_policy"]["reason"] == "operator_setup_env_unresolved"
    assert status["live_policy"]["missing_env_vars"] == ["FAL_KEY"]
    assert status["live_policy"]["operator_setup_actions"] == operator_setup_actions
    assert status["live_policy"]["action_types"] == ["configure_video_fallback_provider"]
    ...
    assert "configure_video_fallback_provider" in status["self_improvement"]["action_types"]
...
def test_visual_self_validation_status_reports_suspended_runtime_policy(tmp_path):
    ...
    payload["runtime_policy"] = {
        ...
        "suspended_action_types": [
            "increase_candidate_budget",
            "prefer_image_first_video",
            "/private/mock-hermes-home/cache/private-prompt.txt",
        ],
        "privacy_safe": True,
    }
    latest_path = _write_latest(tmp_path, payload)
