...
def test_promotion_policy_requires_successful_delivery_evidence():
    from agent.visual.promotion_policy import evaluate_shadow_promotion
    ...
    assert decision.allowed is False
    assert "insufficient_successful_deliveries" in decision.reasons


def test_promotion_policy_blocks_judge_human_disagreement_and_veto():
    from agent.visual.promotion_policy import evaluate_shadow_promotion
    ...
    decision = evaluate_shadow_promotion(
        {
            ...
            "recent_negative_feedback_count": 0,
            "human_veto_count": 1,
            "judge_human_disagreement_rate": 0.60,
            "self_validation_success": True,
        },
        operator_approved=True,
    )
    ...
    assert decision.allowed is False
    assert "human_veto_detected" in decision.reasons
    assert "judge_human_disagreement_above_threshold" in decision.reasons


def test_promotion_policy_blocks_prompt_mutation_requests():
    from agent.visual.promotion_policy import evaluate_shadow_promotion
    ...
    assert decision.allowed is False
    assert "prompt_mutation_requested" in decision.reasons
