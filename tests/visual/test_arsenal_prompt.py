...
def test_arsenal_prompt_variants_turn_xai_art_direction_repairs_into_executable_anime_candidates():
    variants = build_arsenal_prompt_variants(
        base_prompt="動漫圖，固定角色身份，高品質性感一些，產出最佳圖片。",
        strategy_plan={"atom_signatures": []},
        feedback_policy={
            "repair_dimensions": [
                {"dimension": "pose_composition", "quality_issue": "pose_stiff"},
                {"dimension": "camera_composition", "quality_issue": "composition_bad"},
                {"dimension": "lighting_depth", "quality_issue": "lighting_flat"},
                {"dimension": "linework_finish", "quality_issue": "linework_flat"},
                {"dimension": "sensual_outfit_design", "quality_issue": "not_sexy_enough"},
            ]
        },
    ...
    prompts = [variant["prompt"] for variant in variants]
    combined = "\n".join(prompts)
    assert any("contrapposto" in prompt and "torso twist" in prompt for prompt in prompts)
    assert "off-axis low-angle three-quarter camera" in combined
    assert "warm key light" in combined
    assert "variable tapered line weight" in combined
    assert "specific seductive outfit construction" in combined
    assert all("top-tier" not in prompt.lower() for prompt in prompts[1:])
