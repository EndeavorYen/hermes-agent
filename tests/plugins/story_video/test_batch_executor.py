...
def test_executor_migrates_pre_alignment_sequence_rescue_once(tmp_path) -> None:
    ...
    image.write_bytes(b"pre-alignment-selected-artifact")
    row = {
        "shot_id": shot["shot_id"],
        "candidate_id": "S00_SH00_PRE_ALIGNMENT",
        "selected": True,
        "status": "selected_current",
        ...
        "hard_blockers": [],
        "vision_evidence": {"status": "PASS", "response_id": "pre-alignment-qc"},
    }
    manifests = context.project_dir / "manifests"
    manifests.mkdir(parents=True, exist_ok=True)
    (manifests / "shot_candidate_manifest.json").write_text(
    ...
    (manifests / "batch_run_manifest.json").write_text(
        json.dumps({"sequence_rescue_attempted_shot_ids": [shot["shot_id"]]}),
        encoding="utf-8",
    )
    generator = FakeGenerator(tmp_path)
    executor = StoryVideoBatchExecutor(
    ...
    bounded = executor.run_chunk(context)

    assert summary.wave == "sequence_rescue"
    assert summary.attempted_shots == (shot["shot_id"],)
    assert bounded.work_status == "terminal_required"
    assert bounded.attempted_shots == ()
    assert len(generator.calls) == 1
    saved = json.loads((manifests / "batch_run_manifest.json").read_text())
    assert saved["sequence_quality_gate_version"] == 2
    assert saved["sequence_quality_gate_migrated_attempted_shot_ids"] == [
        shot["shot_id"]
    ]
