...
class AIAgent:
    ...
    def __init__(
        ...
        # Thin deterministic reflection artifacts for complex / failed turns.
        self._reflection_enabled = False
        self._reflection_min_tool_turns = 2
        self._reflection_max_user_message_chars = 500
        self._reflection_max_final_response_chars = 300
        try:
            reflection_config = _agent_cfg.get("reflection", {})
            if not isinstance(reflection_config, dict):
                reflection_config = {}
            self._reflection_enabled = bool(reflection_config.get("enabled", False))
            self._reflection_min_tool_turns = int(reflection_config.get("min_tool_turns", 2))
            self._reflection_max_user_message_chars = int(reflection_config.get("max_user_message_chars", 500))
            self._reflection_max_final_response_chars = int(reflection_config.get("max_final_response_chars", 300))
        except Exception:
            pass
    ...
    def run_conversation(
        ...
        _background_review_queued = bool(
            final_response and not interrupted and (_should_review_memory or _should_review_skills)
        )
        ...
        if should_emit_reflection(
            enabled=self._reflection_enabled,
            messages=messages,
            completed=completed,
            interrupted=interrupted,
            partial=result["partial"],
            min_tool_turns=self._reflection_min_tool_turns,
        ):
            try:
                reflection_record = build_reflection_record(
                    ...
                    interrupted=interrupted,
                    partial=result["partial"],
                    max_user_message_chars=self._reflection_max_user_message_chars,
                    max_final_response_chars=self._reflection_max_final_response_chars,
                    post_response_review={
                        "memory": _should_review_memory,
                        "skills": _should_review_skills,
                        "queued": _background_review_queued,
                    },
                )
                append_reflection_record(reflection_record)
            except Exception as exc:
                logger.debug("Reflection artifact write failed: %s", exc)
