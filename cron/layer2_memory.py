"""SQLite-backed Layer-2 sidecar memory for cron memory-pipeline jobs.
...
- parses fenced Layer-2 payloads from cron final responses
...
class Layer2Store:
    """SQLite-backed candidate ledger for cron-derived memory signals."""
    ...
    def get_candidate(self, canonical_text: str) -> Optional[Dict[str, Any]]:
        with self._connect() as conn:
            row = conn.execute(
                "SELECT * FROM candidates WHERE canonical_text = ?",
    ...
    def record_event(
        ...
        *,
        ...
        support_delta = 1 if counts_for_recurrence and event_name in {"create", "strengthen"} else 0
        contradict_delta = 1 if counts_for_recurrence and event_name == "contradict" else 0
        next_status = status
        ...
        with self._connect() as conn:
            ...
            conn.execute(
                ...
                (
                    ...
                    source_ref,
                    source_event_id,
                    1 if counts_for_recurrence else 0,
                    support_delta,
                    contradict_delta,
