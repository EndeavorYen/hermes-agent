...
def _replace_terminal_assistant_response(messages, final_response: str) -> None:
    ...
    *,
    ...
    try:
        ...
        if final_response and not interrupted and preserved_verification_fallback:
            _replace_terminal_assistant_response(messages, final_response)
        ...
        try:
            ...
            for _hook_result in _transform_results:
                ...
                if isinstance(_hook_result, Mapping):
                    transformed = str(
                        _hook_result.get("response_text")
                        or _hook_result.get("final_response")
                        or ""
                    )
                    if transformed:
                    ...
                    turn_control_status = str(
                        _hook_result.get("turn_control_status")
                        or _hook_result.get("status")
                        or "applied"
                    )
                    if _hook_result.get("completed") is False:
                        turn_control_completed = False
                        completed = False
                    if transformed:
                        break
            if _response_transformed and final_response:
                _replace_terminal_assistant_response(messages, final_response)
