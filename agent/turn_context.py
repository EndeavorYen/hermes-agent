    ...
    *,
    ...
    try:
        if isinstance(original_user_message, str):
            ...
            from agent.visual.prompt_arsenal import build_prompt_arsenal_context
            from agent.visual.prompt_arsenal import prompt_requests_arsenal_lookup
            from agent.visual.tracking import default_visual_ledger_path

            if prompt_requests_arsenal_lookup(original_user_message):
                ledger = VisualAttemptLedger(default_visual_ledger_path())
                _arsenal_context = build_prompt_arsenal_context(
                    ledger,
                ...
                if _arsenal_context:
                    plugin_user_context = "\n\n".join(
                        part for part in (plugin_user_context, _arsenal_context) if part
                    )
    except Exception as exc:
        logger.debug("visual prompt arsenal context skipped: %s", exc)
