...
def build_raphael_control_decision(
    ...
    *,
    ...
    reference_count = len(attachment_list)
    missing_reference = _missing_reference_index(prompt, reference_count)
    if missing_reference is not None:
        question = (
            f"你提到 ref{missing_reference}，但目前只看到 "
            f"{reference_count} 個 reference；請補上 ref{missing_reference} "
            "或說明要改用哪一張。"
        )
        return RaphaelControlDecision(
            mode="needs_clarification",
