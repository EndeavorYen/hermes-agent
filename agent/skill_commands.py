...
def resolve_bare_skill_invocation(text: str) -> Optional[tuple[str, str]]:
    ...
    This supports chat surfaces where the user should be able to trigger a
    skill without typing a slash command manually. To avoid accidental matches,
    only two forms are recognized:

    1. the entire message is exactly the skill slug
    2. the first line is exactly the skill slug and later lines carry the
       user instruction
    """
