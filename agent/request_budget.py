"""Per-turn request budget diagnostics.

This module keeps timing and payload-size accounting out of the model hot path.
It records coarse buckets only: enough to explain latency without changing agent
behavior or leaking request bodies into logs.
"""
...
def estimate_tool_schema_tokens(tools: Any) -> tuple[int, int, int]:
    """Return (schema_count, estimated_tokens, byte_count) for tool schemas."""
...
def estimate_skill_index_tokens(prompt: str | None) -> int:
    """Estimate the token weight of the rendered skills index inside a prompt."""
    if not prompt:
        return 0
...
class RequestBudget:
    ...
    def mark_model_first_byte(self, *, source: str = "stream_delta") -> None:
        ...
        ttfb_ms = max(0, int(round((self._active_first_byte_at - self._active_model_start) * 1000)))
    ...
    def mark_model_request_end(self) -> None:
        ...
        elapsed_ms = max(0, int(round((time.perf_counter() - self._active_model_start) * 1000)))
    ...
    def snapshot(self, *, reason: str, api_calls: int) -> dict[str, Any]:
        return {
            ...
            "reason": reason,
            "api_calls": api_calls,
            "total_ms": max(0, int(round((time.perf_counter() - self.started_at) * 1000))),
