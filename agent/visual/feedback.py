...
def _extract_issues(text: str) -> list[str]:
    ...
    if _has_any(text, ("動作", "姿勢", "pose")) and _has_any(
        text,
        ("生硬", "僵硬", "死板", "stiff", "rigid"),
    ):
        issues.append("pose_stiff")
    if "構圖" in text and _has_any(text, ("差", "不好", "普普", "無新意")):
        issues.append("composition_bad")
    if _has_any(text, ("鏡頭", "camera", "角度")) and _has_any(
        text,
        ("死板", "無聊", "平", "沒有變化", "boring", "flat"),
    ):
        issues.append("composition_bad")
    if _has_any(text, ("光影", "光線", "lighting")) and _has_any(
        text,
        ("平", "平面", "扁平", "沒有特殊", "不好看", "flat"),
    ):
        issues.append("lighting_flat")
    if _has_any(text, ("線條", "linework", "line art")) and _has_any(
        text,
        ("平", "平面", "粗糙", "粗", "不精緻", "flat", "rough"),
    ):
        issues.append("linework_flat")
