...
"""
    ...
    loop_parser = subparsers.add_parser(
        "loop",
        help="Run one bounded autonomous continuation pass",
        description="Inspect a target session and recent runtime artifacts, let Hermes decide stop/continue, and optionally execute one continuation step.",
    )
    loop_subparsers = loop_parser.add_subparsers(dest="loop_command")
    loop_once = loop_subparsers.add_parser("once", help="Decide and optionally execute one continuation step")
    ...
    loop_once.add_argument("-c", "--continue", dest="continue_last", action="store_true", help="Use the most recent CLI session")
    loop_once.add_argument("--dry-run", action="store_true", help="Show the decision and next prompt without executing the continuation step")
    loop_once.add_argument("--max-cycles", type=int, default=1, help="Maximum bounded continuation cycles to run before stopping (default: 1)")
    ...
    loop_run = loop_subparsers.add_parser("run", help="Repeat bounded continuation passes until stop/error/max-runs")
    ...
    loop_run.add_argument("-c", "--continue", dest="continue_last", action="store_true", help="Use the most recent CLI session")
    loop_run.add_argument("--dry-run", action="store_true", help="Run exactly one dry-run continuation decision and stop")
    loop_run.add_argument("--max-runs", type=int, default=3, help="Maximum outer launcher runs before stopping (default: 3)")
