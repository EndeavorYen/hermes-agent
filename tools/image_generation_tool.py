...
def image_generate_tool(
    ...
    ``reference_images``. Direct Python callers can also pass reference aliases
    (``input_image``, ``input_images``, ``image_style_references``) plus legacy
    tuning overrides. Reference inputs are either mapped to a model-supported
    style-reference field or rejected explicitly; non-reference tuning overrides
    still pass through the per-model ``supports`` whitelist.
    ...
    try:
        ...
        response_data = {
            ...
            "reference_conditioning": "fal_image_style_references" if style_reference_images else "none",
        }
...
def _dispatch_to_plugin_provider(prompt: str, aspect_ratio: str, **extra_args):
    ...
    try:
        ...
        for key in ("reference_images", "input_image", "input_images", "image_style_references"):
